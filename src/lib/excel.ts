...
export async function generateBudgetReportExcel(
    ...
    // Add remaining balance row for ACBS reports
    if (corpusAmount !== undefined) {
        const remainingBalance = corpusAmount - totalAmount;
        const balanceRow = worksheet.addRow(['', '', 'Remaining Balance:', remainingBalance, '', '', '']);
        balanceRow.getCell(3).font = { bold: true, color: { argb: remainingBalance >= 0 ? 'FF047857' : 'FFDC2626' } };
        balanceRow.getCell(4).font = { bold: true, color: { argb: remainingBalance >= 0 ? 'FF047857' : 'FFDC2626' } };
...
export async function generateExpenseReportExcel(
    ...
    if (corpusAmount !== undefined) {
        const remainingBalance = corpusAmount - totalAmount;
        const balanceRow = worksheet.addRow(['', 'Remaining Balance:', remainingBalance, '', '', '', '', '']);
        balanceRow.getCell(2).font = { bold: true, color: { argb: remainingBalance >= 0 ? 'FF047857' : 'FFDC2626' } };
        balanceRow.getCell(3).font = { bold: true, color: { argb: remainingBalance >= 0 ? 'FF047857' : 'FFDC2626' } };
